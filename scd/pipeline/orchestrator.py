...
async def run_pipeline(repo_a_path: str, repo_b_path: str, config: ScdConfig) -> ScdReport:
    ...
    selected_matches = filter_matches_by_depth(filtered_matches, config.dir_match_depth)
    depth_dropped = len(filtered_matches) - len(selected_matches)
    if depth_dropped:
        if config.dir_match_depth == "highest":
            console.print(
                f"  Dropped {depth_dropped} descendant dir pair(s) covered by "
                f"higher-level matches (keeping {len(selected_matches)})"
            )
        else:
            console.print(
                f"  Dropped {depth_dropped} ancestor dir pair(s) superseded by "
                f"more specific matches (keeping {len(selected_matches)})"
            )

    # Build file pairs and write compared_pairs.txt
    all_file_pairs = build_all_file_pairs(selected_matches, repo_a, repo_b)
